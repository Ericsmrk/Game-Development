...
//
	...
	//resize funcion
	Scene->ResizeGLScene(width,height);//sets keyboard to focus

	//
	if(!Scene->DrawGLScene()){
...
{
    //Scene->winMsg(hWnd,uMsg,wParam,lParam); //CALL ADDED HERE
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow((char*)"Game Engine Lesson 01",fullscreenWidth,fullscreenHeight,256,fullscreen))
	{
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		//if (keys[VK_ESCAPE]|| !Scene->DrawGLScene())
		if (keys[VK_ESCAPE])
			{
